...
int factory_test()
{
  std::map<std::string, std::string> con = { {"DollarToEuro", "Dollar to Euro Converter"},
                {"DollarToPound", "Dollar to Pound Converter"},
                {"DollarToSwissFranc", "Dollar to SwissFranc Converter"},
                {"MeterToMile", "Meter to Mile Converter"},
                {"MeterToFeet", "Meter to Feet Converter"},
                {"CelsiusToFahrenheit", "Celsius to Fahrenheit Converter"},
                {"CelsiusToKelvin", "Celsius to Kelvin Converter"}};

   for (auto& x: con)
  {
     std::shared_ptr<UnitConverter> converter = Factory::instance()->create(x.first);
      TINYTEST_ASSERT(converter->toString() == x.second);
...
TINYTEST_START_SUITE(Converter);
 /* TINYTEST_ADD_TEST(dollarToEuroTest);
  ...
  TINYTEST_ADD_TEST(factory_test);
TINYTEST_END_SUITE();
